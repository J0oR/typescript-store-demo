...
const products = [];
const processoRiciclo = new ProcessoProduzione(dettagliProcesso.nome, dettagliProcesso.descrizione);
const clients = [];
const orders = [];
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    if (productForm) {
        productForm.addEventListener("submit", (event) => {
            ...
            let newProduct = handleProductFormSubmit(event); // Call the function to get the product
    ...
    if (clientForm) {
        clientForm.addEventListener("submit", (event) => {
            let newClient = handleClientFormSubmit(event);
            const emailExists = clients.some((client) => client.email === newClient.email);
            if (!emailExists) {
